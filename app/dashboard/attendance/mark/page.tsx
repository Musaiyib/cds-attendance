...
export default function add() {
  ...
  const handleUpdateAttendance = async (
    ...
    const res = await updateAttendance(corpId, attendance);
    if (res.code === 200) {
                    ...
                    corps.map((corp: CorpInterface, corpIndex: number) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3  flex justify-around">
                          {Object.entries(corp.attendance).map(
                            ([key, value], weekIndex: number) => (
                              <label key={weekIndex} className="flex flex-col">
                                <input
                                  type="checkbox"
                                  checked={value}
                                  onChange={() => {
                                    const updatedAttendance = {
                                      ...corp.attendance,
                                      [key]: !value, // Toggle the value using the 'key' variable
                                    };
                                    const updatedCorps = [...corps];
                                    updatedCorps[corpIndex] = {
                                      ...corp,
